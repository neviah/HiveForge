...
function inferGameStudioScaffoldKind(projectState) {
  const source = `${String(projectState && projectState.name || '')} ${String(projectState && projectState.goal || '')}`.toLowerCase();
  if (/\b(minesweeper|mines)\b/.test(source)) return 'mines';
  if (/\bsolitaire\b/.test(source)) return 'solitaire';
  return 'generic';
}

function shouldRewriteScaffoldFile(filePath, legacyNeedle = '') {
  if (!fs.existsSync(filePath)) return true;
  try {
    const raw = fs.readFileSync(filePath, 'utf-8');
    if (legacyNeedle && raw.includes(legacyNeedle)) return true;
    return /generated by hiveforge/i.test(raw);
  } catch {
    return false;
  }
}

function buildGameStudioScaffold(projectState) {
  const kind = inferGameStudioScaffoldKind(projectState);
  const projectLabel = String(projectState && projectState.name ? projectState.name : 'Game').trim() || 'Game';
...
<head>
  ...
  <meta name="generator" content="generated by HiveForge" />
  <title>HiveForge Mines</title>
  <link rel="stylesheet" href="./style.css" />
</head>
<body>
  <main class="app">
    <h1>Mines (MVP)</h1>
    <p class="sub">Playable local scaffold generated by HiveForge.</p>
    <div class="hud">
      <span id="statusText">Clear all safe tiles.</span>
      <button id="restartBtn">Restart</button>
    </div>
    <section id="board" class="board" aria-label="Mines board"></section>
  </main>
  <script src="./game.js"></script>
</body>
</html>
`,
      styleCss: `/* generated by HiveForge scaffold */
:root {
  color-scheme: dark;
  --bg: #0b1220;
  --panel: #111c33;
...
body { margin: 0; font-family: "Segoe UI", sans-serif; background: radial-gradient(circle at top, #1d3158, var(--bg)); color: var(--ink); }
.app { max-width: 760px; margin: 0 auto; padding: 1.1rem; }
.sub { color: #b6c2d8; }
.hud { display: flex; align-items: center; justify-content: space-between; gap: 0.6rem; margin: 0.8rem 0; }
.board { display: grid; grid-template-columns: repeat(9, minmax(0, 1fr)); gap: 0.3rem; max-width: 560px; }
.cell { border: 1px solid #2f4672; background: #23385f; color: #f3f7ff; border-radius: 6px; min-height: 42px; font-weight: 700; cursor: pointer; }
.cell.revealed { background: var(--panel); cursor: default; }
.cell.mine { background: var(--danger); color: #160000; }
button { background: var(--accent); color: #092033; border: 0; border-radius: 8px; padding: 0.45rem 0.85rem; font-weight: 700; cursor: pointer; }
`,
      gameJs: `// generated by HiveForge scaffold
const SIZE = 9;
const BOMBS = 10;
let grid = [];
let gameOver = false;
...
function buildGrid() {
  grid = Array.from({ length: SIZE * SIZE }, () => ({ mine: false, count: 0, revealed: false }));
  let placed = 0;
  while (placed < BOMBS) {
    const pick = Math.floor(Math.random() * grid.length);
    if (grid[pick].mine) continue;
    grid[pick].mine = true;
    placed += 1;
  }
    ...
    for (let c = 0; c < SIZE; c++) {
      ...
      for (let dr = -1; dr <= 1; dr++) {
        for (let dc = -1; dc <= 1; dc++) {
          ...
          const nr = r + dr;
          const nc = c + dc;
          if (inBounds(nr, nc) && grid[idx(nr, nc)].mine) count += 1;
        }
      }
      cell.count = count;
    }
...
function checkWin() {
  ...
  const safeRevealed = grid.filter((c) => !c.mine && c.revealed).length;
  if (safeRevealed === safeTotal) {
    gameOver = true;
    setStatus('You win. All safe tiles cleared.');
  }
}
...
function render() {
  ...
  for (let r = 0; r < SIZE; r++) {
    for (let c = 0; c < SIZE; c++) {
      ...
      btn.className = 'cell' + (cell.revealed ? ' revealed' : '') + (cell.revealed && cell.mine ? ' mine' : '');
      if (cell.revealed) {
        btn.textContent = cell.mine ? 'X' : (cell.count ? String(cell.count) : '');
      } else {
        btn.textContent = '';
...
function restart() {
  gameOver = false;
  buildGrid();
  setStatus('Clear all safe tiles.');
  render();
}
...
restart();
`,
      readme: `# Mines MVP Scaffold

This folder is generated by HiveForge when using the Game Studio template.
...
- This is the baseline playable scaffold generated by HiveForge.
- Agents can iterate on this implementation during project execution.
`,
    };
  }
...
<head>
  ...
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <meta name="generator" content="generated by HiveForge" />
  <title>HiveForge Solitaire</title>
  <link rel="stylesheet" href="./style.css" />
</head>
<body>
  <main class="app">
    <h1>Solitaire (MVP)</h1>
    <p class="sub">Playable local scaffold generated by HiveForge.</p>
    <section id="board" class="board" aria-label="Solitaire board"></section>
    <div class="actions">
      <button id="restartBtn">Restart</button>
    </div>
  </main>
...
* { box-sizing: border-box; }
body { margin: 0; font-family: "Segoe UI", sans-serif; background: radial-gradient(circle at top, #223b5f, var(--bg)); color: var(--ink); }
.app { max-width: 980px; margin: 0 auto; padding: 1.2rem; }
.sub { color: #b7c5d6; }
.board { display: grid; grid-template-columns: repeat(7, 1fr); gap: 0.6rem; margin-top: 1rem; }
.pile { background: var(--panel); min-height: 180px; border: 1px solid #334b6d; border-radius: 10px; padding: 0.4rem; }
.card { background: #f5f7fa; color: #111; border-radius: 6px; padding: 0.35rem 0.45rem; margin-bottom: 0.3rem; font-weight: 600; }
.actions { margin-top: 1rem; }
button { background: var(--accent); color: #141414; border: 0; border-radius: 8px; padding: 0.5rem 0.8rem; font-weight: 700; cursor: pointer; }
`,
      gameJs: `// generated by HiveForge scaffold
const suits = ["S", "H", "D", "C"];
...
<head>
  ...
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <meta name="generator" content="generated by HiveForge" />
  <title>HiveForge ${projectLabel}</title>
  <link rel="stylesheet" href="./style.css" />
</head>
<body>
  <main class="app">
    <h1>${projectLabel} (MVP)</h1>
    <p class="sub">Playable local scaffold generated by HiveForge.</p>
...
* { box-sizing: border-box; }
body { margin: 0; font-family: "Segoe UI", sans-serif; background: radial-gradient(circle at top, #1e3a8a, var(--bg)); color: var(--ink); }
.app { max-width: 880px; margin: 0 auto; padding: 1.2rem; }
.sub { color: #b8c5db; }
.board { display: grid; grid-template-columns: repeat(4, 1fr); gap: 0.7rem; margin-top: 1rem; }
.tile { background: var(--panel); border: 1px solid #334155; border-radius: 10px; min-height: 96px; display: grid; place-items: center; font-weight: 700; }
.actions { margin-top: 1rem; }
button { background: var(--accent); color: #022d36; border: 0; border-radius: 8px; padding: 0.5rem 0.9rem; font-weight: 700; cursor: pointer; }
`,
    gameJs: `// generated by HiveForge scaffold
function render() {
  const board = document.getElementById('board');
...
render();
`,
    readme: `# ${projectLabel} MVP Scaffold
...
function writeProjectWorkspaceArtifacts(projectState) {
  ...
  if (String(projectState.template || '').toLowerCase() === 'game_studio') {
    ...
    const scaffold = buildGameStudioScaffold(projectState);
    const forceMinesMigration = scaffold.kind === 'mines';

    if (shouldRewriteScaffoldFile(indexPath, forceMinesMigration ? 'Solitaire (MVP)' : '')) {
      fs.writeFileSync(indexPath, scaffold.indexHtml, 'utf-8');
    }

    if (shouldRewriteScaffoldFile(cssPath, forceMinesMigration ? 'grid-template-columns: repeat(7, 1fr)' : '')) {
      fs.writeFileSync(cssPath, scaffold.styleCss, 'utf-8');
    }

    if (shouldRewriteScaffoldFile(jsPath, forceMinesMigration ? 'const suits = [' : '')) {
      fs.writeFileSync(jsPath, scaffold.gameJs, 'utf-8');
    }

    if (shouldRewriteScaffoldFile(readmePath, forceMinesMigration ? '# Solitaire MVP Scaffold' : '')) {
      fs.writeFileSync(readmePath, scaffold.readme, 'utf-8');
    }
